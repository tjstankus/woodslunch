...
describe StudentOrder do
  ...
  describe '.new' do
    ...
    context 'given today is the first of the month' do

      it 'sets starts_on to the first date of this month' do
        Date.stub(:today).and_return(Date.parse('2011-09-01'))
        StudentOrder.new.starts_on.should == Date.parse('2011-09-01')
      end

      it 'sets ends_on to the last date of this month' do
        Date.stub(:today).and_return(Date.parse('2011-09-01'))
        StudentOrder.new.ends_on.should == Date.parse('2011-09-30')
      end
    end
  ...
  describe '.new_from_params' do
    ...
    context 'given year and month' do
      ...
      it 'sets ends_on' do
        student_order.starts_on.should == Date.parse("#{year}-#{month}-31")
      end
    end
  end
