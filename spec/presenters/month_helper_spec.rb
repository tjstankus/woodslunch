...
describe MonthHelper, :wip => true do
  ...
  describe '#month' do
    context 'when initialized with number' do
      it 'returns numeric month' do
        MonthPresenter.new(5, 2011).month.should == 5
      end
    end
  ...
  describe '#year' do
    context 'when initialized with number' do
      it 'returns numeric year' do
        MonthPresenter.new(5, 2011).year.should == 2011
      end
    end
  ...
  describe '#weekdays_grouped_by_week' do
    context 'given a month where first weekday is a Friday' do
      ...
      let(:presenter) { presenter = MonthPresenter.new(7, 2011) }
  ...
  describe '#day' do
    describe '#month_day' do
      it 'returns numeric day of month' do
        presenter = MonthPresenter.new(7, 2011)
